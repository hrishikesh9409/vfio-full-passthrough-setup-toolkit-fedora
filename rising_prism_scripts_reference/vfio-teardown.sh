...
## === Begin: explicit return to NVIDIA ===
GPU="0000:08:00.0"
HDA="0000:08:00.1"
...
for dev in "$GPU" "$HDA"; do
  if [ -e "/sys/bus/pci/devices/$dev/driver/unbind" ]; then
    ...
    echo "$DATE Unbound $dev from vfio-pci"
  fi
done

# Ensure nvidia core module is present so /sys/bus/pci/drivers/nvidia exists
modprobe nvidia || true

# Rebind to NVIDIA driver (either echo to bind or use new_id fallback)
for dev in "$GPU" "$HDA"; do
  if [ -e "/sys/bus/pci/drivers/nvidia/bind" ]; then
    echo "$dev" > /sys/bus/pci/drivers/nvidia/bind 2>/dev/null || true
  else
    # Fallback path: teach the driver the IDs, then it should auto-attach
    echo 10de 2208 > /sys/bus/pci/drivers/nvidia/new_id 2>/dev/null || true
    echo 10de 1aef > /sys/bus/pci/drivers/nvidia/new_id 2>/dev/null || true
  fi
done
## === End: explicit return to NVIDIA ===
